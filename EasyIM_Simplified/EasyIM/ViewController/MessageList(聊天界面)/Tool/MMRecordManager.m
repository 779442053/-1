...
@implementation MMRecordManager
...
{
    return [MMFileTool createPathWithChildPath:kChildPath];
}
                           ...
                           successfully:(BOOL)flag
{
    ...
    CGFloat time = [self durationWithVideo:[_recorder url]];
    if (ceilf(time) < 1.0) {//四舍五入
        [MBProgressHUD showMessage:@"语音时间太短，发送失败！" delay:1];
        [self cancelCurrentRecording];
        return;
    }else if (ceil(time) >= 60.0) {
        [MBProgressHUD showMessage:@"语音时间太长，发送失败！" delay:1];
        [self cancelCurrentRecording];
        return;
    }
