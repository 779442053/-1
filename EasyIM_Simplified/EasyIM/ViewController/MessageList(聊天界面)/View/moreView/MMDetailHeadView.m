...
#define MMHeadMaxCols 4

@interface MMDetailHeadView ()
...
- (void)headBtnClicked:(MMDetailButton *)headBtn
{
...
- (void)setUsers:(NSArray *)users
{
    ...
    for (int i = 0; i < count; i ++) {
        MMUserModel *user            = _users[i];
        MMDetailButton *headBtn = [[MMDetailButton alloc] init];
...
- (void)layoutSubviews
{
    ...
    CGFloat btnW     = (self.width - 2*inset)/MMHeadMaxCols;
    CGFloat btnH     = btnW;
    for (int i = 0; i < count; i ++) {
        MMDetailButton *btn = self.subviews[i];
        btn.width           = btnW;
        btn.height          = btnH;
        btn.x               = inset + (i % MMHeadMaxCols)*btnW;
        //        btn.y               = (i / ICHeadMaxCols)*btnH + topImageH;
        btn.y               = (i / MMHeadMaxCols)*btnH;
    }
    self.addBtn.width     = btnW;
    self.addBtn.height    = btnH;
    self.addBtn.x         = inset + (count%MMHeadMaxCols)*btnW;
    //    self.addBtn.y         = (count/ICHeadMaxCols)*btnH + topImageH;
    self.addBtn.y         = (count/MMHeadMaxCols)*btnH;
    
    if (self.isMaster && _users.count > 2) { // 3个人以上
        self.deleteBtn.width     = btnW;
        self.deleteBtn.height    = btnH;
        self.deleteBtn.x         = inset + ((count+1)%MMHeadMaxCols)*btnW;
        //        self.deleteBtn.y         = ((count+1)/ICHeadMaxCols)*btnH + topImageH;
        self.deleteBtn.y         = ((count+1)/MMHeadMaxCols)*btnH;
    }
}
...
- (UIView *)topView
{
    if (!_topView) {
        ...
        _topView.backgroundColor = MMRGB(0x027996);
