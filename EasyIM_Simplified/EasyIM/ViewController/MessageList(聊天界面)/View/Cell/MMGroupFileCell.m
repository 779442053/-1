...
#import "MMGroupFileCell.h"
#import "MMFileTool.h"
#import "NSDate+Extension.h"

@interface MMGroupFileCell ()
...
@implementation MMGroupFileCell
...
+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    ...
    MMGroupFileCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[MMGroupFileCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}


- (void)setMessage:(MMMessage *)message
{
    _message = message;
    
    NSDictionary *dic = [NSDictionary dictionaryWithJsonString:@(message.slice.duration).description];
    NSNumber * size   = [dic objectForKey:@"s"];
    NSNumber * type   = [dic objectForKey:@"x"];
    NSString * name   = [dic objectForKey:@"n"];
    
    self.imageV.image   = [MMMessageHelper allocationImage:type.intValue];
    self.sizeLabel.text = [MMFileTool fileSizeWithInteger:size.integerValue];
    ...
    [self fileExisted:message.slice.content];
...
- (void)layoutSubviews
{
    ...
    _nameLabel.width = KScreenWidth-_imageV.right-10-60;
    ...
    self.seeBtn.frame = CGRectMake(KScreenWidth-15-40, 20, 40, 30);
...
- (void)seeBtnClicked:(UIButton *)btn
{
    if ([self.delegate respondsToSelector:@selector(isFileExisted:isExisted:fileKey:fileName:)]) {
        [self.delegate isFileExisted:self isExisted:[btn.titleLabel.text isEqualToString:@"查看"] fileKey:_message.slice.content fileName:_nameLabel.text];
    }
}
