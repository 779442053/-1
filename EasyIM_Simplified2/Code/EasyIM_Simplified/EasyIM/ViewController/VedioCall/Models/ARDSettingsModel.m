...
- (NSString*)currentUidSettingFromStore {
    if ([[self settingsStore] uid] == nil) {
        int r = arc4random_uniform(100000000);
        [[self settingsStore] setUid:[NSString stringWithFormat:@"%d", r]];
    }
    return [[self settingsStore] uid];
}

- (void)storeUidSetting:(NSString*)uid {
    [[self settingsStore] setUid:uid];
}

- (NSString*)currentRoomUrlSettingFromStore {
//    if ([[self settingsStore] roomUrl] == nil) {
//        [[self settingsStore] setRoomUrl:@"ws://45.61.236.14:8188"];
//    }
//    return [[self settingsStore] roomUrl];
    return [ZWUserModel currentUser].webrtc;
}

- (void)storeRoomUrlSetting:(NSString*)roomUrl {
    [[self settingsStore] setRoomUrl:roomUrl];
}

- (int)currentRidSettingFromStore {
    return 0;
}

- (void)storeRidSetting:(int)rid {
}
