...
@interface MMChatBox ()<UITextViewDelegate>
...
@implementation MMChatBox

- (id) initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        [self setBackgroundColor:MMColor(241, 241, 248)];
        ...
        self.status = MMChatBoxStatusNothing; // 起始状态
...
- (UIView *) topLine
{
    if (_topLine == nil) {
        ...
        [_topLine setBackgroundColor:MMColor(165, 165, 165)];
    }
    return _topLine;
}
...
{
    if (_moreButton == nil) {
        _moreButton = [[UIButton alloc] initWithFrame:CGRectMake(KScreenWidth - CHATBOX_BUTTON_WIDTH, (HEIGHT_TABBAR - CHATBOX_BUTTON_WIDTH) / 2, CHATBOX_BUTTON_WIDTH, CHATBOX_BUTTON_WIDTH)];
...
- (void) textViewDidBeginEditing:(UITextView *)textView
{
    ...
    self.status = MMChatBoxStatusShowKeyboard;
}
...
{
    MMChatBoxStatus lastStatus = self.status;
    if (lastStatus == MMChatBoxStatusShowVoice) {//正在显示talkButton，改为键盘状态
        self.status = MMChatBoxStatusShowKeyboard;
        ...
        self.status = MMChatBoxStatusShowVoice;
        [self.textView resignFirstResponder];
...
{
    MMChatBoxStatus lastStatus = self.status;
    if (lastStatus == MMChatBoxStatusShowMore) { // 当前显示的就是more页面
        self.status = MMChatBoxStatusShowKeyboard;
        [self.textView becomeFirstResponder];
    } else {
        [self.talkButton setHidden:YES];
        ...
        self.status = MMChatBoxStatusShowMore;
        if (lastStatus == MMChatBoxStatusShowFace) {  // 改变按钮样式
            [_faceButton setImage:[UIImage imageNamed:@"ToolViewEmotion"] forState:UIControlStateNormal];
        } else if (lastStatus == MMChatBoxStatusShowVoice) {
            [_talkButton setHidden:YES];
            [_textView setHidden:NO];
            [_voiceButton setImage:[UIImage imageNamed:@"ToolViewInputVoice"] forState:UIControlStateNormal];
        } else if (lastStatus == MMChatBoxStatusShowKeyboard) {
            [self.textView resignFirstResponder];
            self.status = MMChatBoxStatusShowMore;
        }
    }
...
{
    MMChatBoxStatus lastStatus = self.status;
    if (lastStatus == MMChatBoxStatusShowFace) {       // 正在显示表情,改为现实键盘状态
        self.status = MMChatBoxStatusShowKeyboard;
        [_faceButton setImage:[UIImage imageNamed:@"ToolViewEmotion"] forState:UIControlStateNormal];
        [self.textView becomeFirstResponder];
    } else {
...
{
    MMEmotion *emotion = notifi.userInfo[GXSelectEmotionKey];
    if (emotion.code) {
