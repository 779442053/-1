...
- (void)setupUIwithFrame:(CGRect)frame
{
    ...
    self.videoLayerView.frame = CGRectMake(0, 10, KScreenWidth,self.recordBtn.top-20-10);
...
- (void)recordVideoStarted
{
    ...
    self.timeLine.frame = CGRectMake(0,self.videoLayerView.bottom , KScreenWidth, 1);
    ...
    [[MMVideoManager shareManager] startRecordingVideoWithFileName:self.videoName];
}
...
- (void)recordVideoFinished
{
    ...
    if ((double)timeInterval <= kDurationTime) { //小于或等于规定时间
        ...
        [[MMVideoManager shareManager] stopRecordingVideo:^(NSString *path) {
...
- (void)recordVideoCanceled
{
    ...
    [[MMVideoManager shareManager] stopRecordingVideo:^(NSString *path) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[MMVideoManager shareManager] cancelRecordingVideoWithFileName:_videoName];
        });
    }];
}
