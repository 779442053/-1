...
#import "MMChatMessageVoiceCell.h"

#import "VoiceConverter.h"

@interface MMChatMessageVoiceCell ()
...
@implementation MMChatMessageVoiceCell
...
- (void)setModelFrame:(MMMessageFrame *)modelFrame
{
    ...
    self.durationLabel.text = [NSString stringWithFormat:@"%ld''",(long)modelFrame.aMessage.slice.duration];
    
    if (modelFrame.aMessage.isSender) {  // sender
...
//    if (modelFrame.aMessage.status == MMMessageStatus_read) {
//        self.redView.hidden  = YES;
//    } else if (modelFrame.aMessage.status == MMMessageStatus_unRead) {
//        self.redView.hidden  = NO;
//    }
    
    if (modelFrame.aMessage.slice.filePath.length) {
        self.redView.hidden  = YES;
    }else{
        self.redView.hidden  = NO;
    }
...
- (UIView *)redView
{
    if (nil == _redView) {
        ...
        _redView.backgroundColor = MMRGB(0xf05e4b);
    }
    return _redView;
}
